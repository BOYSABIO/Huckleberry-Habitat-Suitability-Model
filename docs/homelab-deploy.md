...
Manual deploy of the API + MLflow stack to a Proxmox LXC. Automated CD is [FUTURE_TASKS.md](../FUTURE_TASKS.md) §1.

For architecture (which model loads, serving vs training, update flows), see [FUTURE_TASKS.md § Architecture](../FUTURE_TASKS.md#architecture-how-serving-training-and-updates-fit-together).
...
**Security:** Ports are exposed on your **home network only**. Do not port-forward to the internet without auth (see FUTURE_TASKS.md for Tailscale/WireGuard).
